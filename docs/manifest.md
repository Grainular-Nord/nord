...
```ts
import { html, mount, css, withScopedStyles } from "@grainular/nord"; 

const styles = css`button { background: red; color: white; } /* Scoped to this component instance! */ `; 

const Button = withScopedStyles(
 ({ label }) => { 
...
const EffectComponent: VoidComponent = () => {
 console.log('I did something') // Logs I did Something
 return null
}

export const App = () => {
 return html`<div>${EffectComponent()}</div>`
}
