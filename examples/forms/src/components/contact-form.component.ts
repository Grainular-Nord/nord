...
export const ContactForm = ({ render, submit }: ContactFormProps) => {
    ...
    return html` <form
        ${on('submit', onSubmit)}
        class="w-full max-w-lg bg-slate-900 rounded-2xl shadow-xl p-8 border border-slate-800 flex flex-col gap-8"
    >
        <div class="mb-2">
    ...
    return html`
        <div class="flex flex-col gap-5">
            ...
            <div>
                ...
                >
                    <span
                        class="text-sm text-slate-300 transition-colors group-data-[touched=true]:group-data-[isValid=false]:text-red-400"
                    >
                    ...
                    <svg
                        class="absolute w-5 h-5 pointer-events-none opacity-0 peer-checked:opacity-100 transition-opacity text-white stroke-current p-0.5"
                        viewBox="0 0 24 24"
                        fill="none"
                        stroke-width="3"
