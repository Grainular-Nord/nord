        ...
        <div data-grainular-editor="preview-toolbar">
            ...
            <div data-grainular-editor="toolbar-tools">
                ...
                ${$each(() => tools)
                    ...
                    .$as(
                        (tool) =>
                            html`<button data-grainular-editor="${tool.id === 'run' ? 'run' : 'tool'}" type="button" title="${tool.title ?? tool.label}" ${on('click', () => void tool.run())}>${tool.label}</button>`,
                    )}
                <button data-grainular-editor="console-toggle" type="button" aria-expanded="${consoleOpen}" ${on('click', () => consoleOpen.update((open) => !open))}>Console${consoleCount}</button>
            </div>
        </div>
        <iframe title="${title ?? 'Nørd playground preview'}" data-grainular-editor="preview-frame" sandbox="allow-scripts allow-same-origin" srcdoc="${preview}" ${receiveMessages}></iframe>
        <div data-grainular-editor="console" role="log" aria-label="Preview console" aria-live="polite">
            <div data-grainular-editor="console-toolbar"><span>Console</span><button type="button" ${on('click', () => previewEvents.set([]))}>Clear</button></div>
            ${$if(hasConsoleOutput)
                .$then(
                    () =>
