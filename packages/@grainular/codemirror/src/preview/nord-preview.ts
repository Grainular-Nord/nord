...
/** A file supplied to the browser-side preview compiler. */
export type ProjectFile = { path: string; contents: string };
/** Console output captured from code running inside the preview iframe. */
export type PreviewEvent = { level: 'error' | 'info' | 'log' | 'warn'; message: string };
/** postMessage payload emitted by the preview shell. */
...
(() => {
    ...
    // This document's origin is opaque (srcdoc), so there is no real parent
    // origin to name here; the receiver re-validates event.source and the
    // session token, which is the boundary that actually matters.
...
/**
 * Builds a sandbox-ready HTML document plus the blob URLs it owns.
 *
 * Browser import maps cannot remap a relative specifier to a blob URL, so the
 * generated module source rewrites known sibling imports directly. Callers
 * must revoke the returned URLs after replacing or disposing the preview.
 */
export const buildPreviewDocument = async (
    ...
    return { document: shell(imports, entryUrl, theme, session), urls: createdUrls };
};
