 ...
 */
export const lifecycleObserver = new (class LifecycleObserver extends MutationObserver {
    ...
    #runMount(node: Node) {
        ...
        this.#unmounts.set(node, new Set([...(this.#unmounts.get(node) ?? []), ...cleanups.filter((cb) => !!cb)]));
    }
